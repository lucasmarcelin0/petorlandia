...
## 2026-10-25 - Filtering Digits and Valid Characters Without Regex
**Learning:** In string normalization routines, replacing `re.sub(r"\D+", "", text)` with `"".join([c for c in text if c.isdigit()])` provides a significant execution speedup (~2x improvement). Similarly, replacing `re.sub(r"[^\d-]", "", raw)` with `"".join([c for c in text if c.isdigit() or c == "-"])` is highly efficient. This avoids the overhead of compiling and evaluating regular expressions.
**Action:** When filtering digits or explicit character sets from short strings (like phone numbers, zip codes, or numeric text), use list comprehensions with `c.isdigit()` or explicit matching instead of `re.sub()`.
