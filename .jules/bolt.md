...
## 2026-09-24 - Pre-compiled O(1) Regex Replacements for Word Substitution
**Learning:** In `services/vacina_pmo_service.py`, `_pmo_clean_address_fragment` contained a slow `O(N)` loop executing 33 `re.sub` calls to replace written number words. Combining them into a single pre-compiled regex using alternation (`|`) ordered by string length (longest-first) and using a custom replacement function (`_pmo_replace_num`) reduced the execution time by ~4x compared to loop execution and overhead elimination. Backreferences in replacement strings should use `\g<1>` to avoid ambiguity.
**Action:** When replacing multiple string variations in an O(N) loop using `re.sub`, combine them into a single pre-compiled regex using alternation and a custom substitution function. Move custom substitution functions to the module level to avoid recreation overhead on every execution.
