...
## 2026-10-26 - Deferring Aggregation Subqueries in Paginated Search Endpoint
**Learning:** In `services/animal_search.py`, `_build_last_appointment_subquery` outer-joined a full table aggregate subquery (`GROUP BY Appointment.animal_id`) before filtering and applying `LIMIT 50`. For default search sorts (`recent_added` and `name_asc`), deferring the appointment lookup until after filtering and limiting matching animal IDs avoids running full-table aggregations over the entire appointment table on every search query.
**Action:** When adding aggregate subqueries for returned list items in search/paginated queries, filter and limit main entities first, then execute targeted aggregate lookups for only those retrieved IDs using `.in_(ids)`.
