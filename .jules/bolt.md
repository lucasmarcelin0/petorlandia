...
## 2026-09-16 - ASCII Fast-Path and Conditional Regex Execution in Medication Search Normalization
**Learning:** In `blueprints/consulta.py`, `_norm_busca` normalized medication titles, active ingredients, and structured fields across dozens or hundreds of search candidate records by executing `unicodedata.normalize("NFKD", str(valor or "").lower())` and character category filtering on every string, as well as running `_RE_NORM_NEOMICINA.sub()` unconditionally. Adding `s.isascii()` to short-circuit NFKD decomposition on pure ASCII strings and guarding regex execution with a `neom`/`neon` substring check yields a ~5x speedup during medication autocomplete search normalization.
**Action:** Always short-circuit ASCII text before unicodedata NFKD decomposition and guard specific regular expression substitutions with string membership checks in autocomplete search normalizers.
