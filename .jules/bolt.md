...
## 2026-09-14 - Pre-compiling Regex Patterns and ASCII Fast-Path in General Text Normalization (app.py)
**Learning:** General text normalization functions in `app.py` like `_normalize_public_text`, `_integration_normalize_match_text` and others executed `unicodedata.normalize("NFKD")` on every text segment, and re-compiled substitution regular expressions (e.g., `\s+` and `[^a-z0-9]+`) continuously. Adding an `isascii()` pre-condition short-circuit and extracting repeating regular expressions out to pre-compiled module-level constants `_RE_SPACES` and `_RE_NON_ALPHANUMERIC` yielded a ~2.7x total speedup (from 3.5s to 1.3s for 100k calls).
**Action:** When creating text normalization helpers that may process millions of entries (like lookup token builders or text match normalizers), pre-compile substitution regexes at the module level and check `.isascii()` to bypass unicodedata NFKD decoding for simple strings.
