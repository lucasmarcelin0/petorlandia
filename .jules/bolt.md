...
## 2026-11-02 - Fast-path Digit Extraction and Whitespace Collapse in PMO Campaign Sync
**Learning:** In `services/vacina_pmo_service.py`, `_normalize_text` and `_digits` are invoked thousands of times during Google Sheets synchronization, address parsing, and phone cleaning. Replacing `re.sub(r"\s+", " ", ...)` with `" ".join(s.split())` yields a ~3.8x speedup. Adding `isinstance(s, str) and s.isdigit()` shortcuts and list comprehension filtering in `_digits` yields a ~3.0x speedup over `re.sub(r"\D+", "", ...)`. Pre-compiling module-level regex patterns for geocoding and video parsing eliminates redundant regex parsing.
**Action:** Use `" ".join(s.split())` for collapsing whitespace, fast-path `isdigit()` checks for extracting numeric characters, and pre-compile module-level regex objects in service helpers.
