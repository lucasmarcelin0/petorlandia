...
@app.route("/endereco_teste", methods=["GET", "POST"])
def endereco_teste():
    if request.method == "POST":
        data = request.form
        ...
        # Verificação e criação do endereço
        ...
        return redirect(url_for("endereco_teste"))

    # Carrega o primeiro endereço salvo (apenas para exemplo)
    endereco = Endereco.query.first()

    # Lista de países para o formulário
    paises = Pais.query.order_by(Pais.nome).all()

    # Estes ajudam a manter os selects após recarregamento
    estados = Estado.query.filter_by(pais_id=endereco.bairro.cidade.estado.pais_id).all() if endereco else []
    cidades = Cidade.query.filter_by(estado_id=endereco.bairro.cidade.estado_id).all() if endereco else []
    bairros = Bairro.query.filter_by(cidade_id=endereco.bairro.cidade_id).all() if endereco else []

    return render_template("teste_endereco.html", paises=paises, endereco=endereco, estados=estados, cidades=cidades, bairros=bairros)





@app.route("/api/estados/<int:pais_id>")
def estados_por_pais(pais_id):
    estados = Estado.query.filter_by(pais_id=pais_id).order_by(Estado.nome).all()
    return jsonify([{"id": e.id, "nome": e.nome} for e in estados])

@app.route("/api/cidades/<int:estado_id>")
def cidades_por_estado(estado_id):
    cidades = Cidade.query.filter_by(estado_id=estado_id).order_by(Cidade.nome).all()
    return jsonify([{"id": c.id, "nome": c.nome} for c in cidades])

@app.route("/api/bairros/<int:cidade_id>")
def bairros_por_cidade(cidade_id):
    bairros = Bairro.query.filter_by(cidade_id=cidade_id).order_by(Bairro.nome).all()
    return jsonify([{"id": b.id, "nome": b.nome} for b in bairros])



@app.route('/api/novo_<tipo>', methods=['POST'])
def adicionar_novo_local(tipo):
    data = request.json
    nome = data.get('nome')
    ...
    if tipo == 'pais':
        existente = Pais.query.filter_by(nome=nome).first()
        if existente:
            return jsonify({'erro': 'País já cadastrado'}), 400
        sigla = data.get('sigla') or nome[:3].upper()
        novo = Pais(nome=nome, sigla=sigla)

    elif tipo == 'estado':
        pais_id = data.get('pais_id')
        if not pais_id:
            return jsonify({'erro': 'pais_id é obrigatório'}), 400

        existente = Estado.query.filter_by(nome=nome, pais_id=pais_id).first()
        if existente:
            return jsonify({'erro': 'Estado já cadastrado neste país'}), 400

        sigla = data.get('sigla') or nome[:2].upper()
        novo = Estado(nome=nome, sigla=sigla, pais_id=pais_id)

    elif tipo == 'cidade':
        estado_id = data.get('estado_id')
        if not estado_id:
            return jsonify({'erro': 'estado_id é obrigatório'}), 400

        existente = Cidade.query.filter_by(nome=nome, estado_id=estado_id).first()
        if existente:
            return jsonify({'erro': 'Cidade já cadastrada neste estado'}), 400

        novo = Cidade(nome=nome, estado_id=estado_id)

    elif tipo == 'bairro':
        cidade_id = data.get('cidade_id')
        if not cidade_id:
            return jsonify({'erro': 'cidade_id é obrigatório'}), 400

        existente = Bairro.query.filter_by(nome=nome, cidade_id=cidade_id).first()
        if existente:
            return jsonify({'erro': 'Bairro já cadastrado nesta cidade'}), 400

        novo = Bairro(nome=nome, cidade_id=cidade_id)
...
def local_por_cep():
    ...
    estado = Estado.query.filter(Estado.sigla == uf).first()
    cidade = Cidade.query.filter_by(nome=cidade_nome).first()
    bairro = Bairro.query.filter_by(cidade_id=cidade.id).first() if cidade else None
