...
# Twilio saiu do requirements (2026-07-10): WhatsApp é via links wa.me. Os
# caminhos _send_share_sms/enviar_mensagem_whatsapp degradam com erro claro se
# as env vars TWILIO_* forem configuradas sem o pacote instalado.
# O placeholder mantém o atributo no módulo para monkeypatch de testes.
Client = None  # resolvido em runtime: twilio.rest.Client (se instalado)
...
# Controle de acesso (get_*_or_404, visibilidade e shares) vive em
# access_control.py; reexportado aqui para as views e para monkeypatch de testes.
from access_control import (  # noqa: E402,F401
    _can_view_user,
    _collect_clinic_ids,
    _is_tutor_portal_user,
    _log_data_share,
    _resolve_shared_access_for_animal,
    _resolve_shared_access_for_consulta,
    _resolve_shared_access_for_user,
    _shared_user_clause,
    _user_visibility_clause,
    _veterinarian_accessible_clinic_ids,
    _viewer_operational_clinic_ids,
    _viewer_parties,
...
def enviar_mensagem_whatsapp(texto: str, numero: str) -> None:
    ...
    if client_cls is None:
        try:
            from twilio.rest import Client as client_cls
        except ImportError as exc:  # pacote removido do requirements em 2026-07-10
            raise RuntimeError(
                "Pacote twilio não instalado. Reinstale (pip install twilio) e "
                "recoloque no requirements.txt para usar envio via WhatsApp/SMS."
            ) from exc
    client = client_cls(account_sid, auth_token)
...
def _appointment_request_within_vet_schedule(veterinario_id, scheduled_date, scheduled_time):
    ...
    for horario in horarios:
        raw_dia = (horario.dia_semana or "").lower()
        if raw_dia.isascii():
            dia = raw_dia
        else:
            dia = unicodedata.normalize("NFKD", raw_dia)
            dia = "".join([ch for ch in dia if not unicodedata.combining(ch)])
        dia = _RE_NON_ALPHANUMERIC.sub(" ", dia).strip()
