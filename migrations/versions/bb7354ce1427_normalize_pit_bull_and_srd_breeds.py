...
Revision ID: bb7354ce1427
Revises: cf857f88b021
Create Date: 2026-07-07 11:50:07.129500
...
revision = 'bb7354ce1427'
down_revision = 'cf857f88b021'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    # "Pit Bull Terrier" estava sendo usado como guarda-chuva para uma raça
    # que na verdade não existe assim. Separa em Pit Bull e Bull Terrier
    # (raças distintas), preservando os animais existentes como Pit Bull
    # (nunca havia dado de origem indicando Bull Terrier de fato).
    ...
    # SRD aparece com vários aliases e até em espécies diferentes (Cachorro,
    # Gato, Outro). Para o cadastro, uma opção única "SRD" é suficiente.
    breeds = conn.execute(sa.text("SELECT id, name, species_id FROM breed ORDER BY id")).fetchall()
