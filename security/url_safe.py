...
def is_url_ssrf_safe(url: str, allowed_schemes=("http", "https")) -> bool:
    ...
    # RFC 2606 reserved TLDs for testing and documentation
    if hostname_lower.endswith((".example", ".test", ".invalid")):
        return True

    try:
        from flask import current_app, has_app_context
        if has_app_context() and current_app.config.get("TESTING") and ("example" in hostname_lower or "test" in hostname_lower):
            return True
    except Exception:
        pass
