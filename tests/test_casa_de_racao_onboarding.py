...
def test_store_onboarding_completes_account_and_catalog(app, client):
    ...
    assert "Vamos deixar sua loja pronta" in response.get_data(as_text=True)
    ...
    response = client.post(
        ...
        data={
            ...
            f"price_{product_ids[1]}": "19.50",
            f"stock_{product_ids[1]}": "8",
        },
    )
    ...
    with app.app_context():
        ...
        assert sorted(product.price for product in products) == [19.5, 89.9]
...
def test_store_onboarding_requires_every_product_price(app, client):
    with app.app_context():
    ...
    response = client.post(
        ...
        data={
            ...
            f"stock_{product_ids[1]}": "0",
        },
    )
    assert response.status_code == 200
    assert "Informe um preço válido para Canex Original." in response.get_data(as_text=True)
