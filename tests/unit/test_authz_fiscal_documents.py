...
from authz import can_view_fiscal_documents, can_manage_fiscal_documents
...
class TestCanViewFiscalDocuments:
    ...
    def test_admin_can_view_any_clinic_documents(self, admin_user):
        assert can_view_fiscal_documents(admin_user, 1)
        assert can_view_fiscal_documents(admin_user, 999)

    def test_owner_can_view_own_clinic_documents(self, owner_user):
        assert can_view_fiscal_documents(owner_user, 1)

    def test_owner_cannot_view_other_clinic_documents(self, owner_user):
        assert not can_view_fiscal_documents(owner_user, 2)

    def test_staff_cannot_view_documents(self, staff_user):
        assert not can_view_fiscal_documents(staff_user, 1)

    def test_vet_cannot_view_documents(self, vet_user):
        assert not can_view_fiscal_documents(vet_user, 1)

    def test_intern_cannot_view_documents(self, intern_user):
        assert not can_view_fiscal_documents(intern_user, 1)

    def test_tutor_cannot_view_documents(self, tutor_user):
...
class TestCanManageFiscalDocuments:
    ...
    def test_admin_can_manage_any_clinic_documents(self, admin_user):
        assert can_manage_fiscal_documents(admin_user, 1)
        assert can_manage_fiscal_documents(admin_user, 999)

    def test_owner_can_manage_own_clinic_documents(self, owner_user):
        assert can_manage_fiscal_documents(owner_user, 1)

    def test_owner_cannot_manage_other_clinic_documents(self, owner_user):
        assert not can_manage_fiscal_documents(owner_user, 2)

    def test_staff_cannot_manage_documents(self, staff_user):
        assert not can_manage_fiscal_documents(staff_user, 1)

    def test_vet_cannot_manage_documents(self, vet_user):
        assert not can_manage_fiscal_documents(vet_user, 1)

    def test_intern_cannot_manage_documents(self, intern_user):
        assert not can_manage_fiscal_documents(intern_user, 1)

    def test_tutor_cannot_manage_documents(self, tutor_user):
