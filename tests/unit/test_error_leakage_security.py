...
def test_arquivar_animal_does_not_leak_exception_details(app, client):
    ...
    with app.app_context():
        vet = User(name="Vet Test 2", email="vet_leak2@test.com", role="admin", worker="veterinario")
        vet.set_password("pass123")
        vet_profile = Veterinario(user=vet, crmv="12347")
        tutor = User(name="Tutor Test 3", email="tutor_leak3@test.com", role="adotante")
        tutor.set_password("pass123")
        animal = Animal(name="Thor Leak", owner=tutor, is_alive=True)
        db.session.add_all([vet, vet_profile, tutor, animal])
        db.session.commit()
        vet_id = vet.id
        animal_id = animal.id
    ...
    with patch.object(db.session, "commit", side_effect=RuntimeError("SECRET_DB_CONSTRAINT_ERROR_ABCDE")):
        response = client.post(
        ...
        assert response.status_code == 400
        json_data = response.get_json()
        assert "SECRET_DB_CONSTRAINT_ERROR_ABCDE" not in json_data["message"]
